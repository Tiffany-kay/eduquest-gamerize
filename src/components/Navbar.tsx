...
import { Book, Lightbulb, LayoutDashboard, Menu, X } from "lucide-react";
    ...
    >
      <nav className="container mx-auto flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <div className="relative w-8 h-8 bg-gradient-to-br from-purple-500 to-purple-700 rounded-lg flex items-center justify-center animate-pulse-soft">
          ...
          <div className="flex flex-col p-4 space-y-3">
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                className={cn(
                  "px-4 py-3 rounded-lg flex items-center text-sm font-medium transition-colors",
                  location.pathname === link.path
                    ? "bg-primary/10 text-primary"
                    : "text-foreground/70 hover:text-foreground hover:bg-muted"
                )}
